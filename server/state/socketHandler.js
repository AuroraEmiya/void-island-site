...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    current_avatar TEXT DEFAULT 'MGMT_SakurabaEma',
    unlocked_avatars TEXT DEFAULT '["default", "XingHui", "YingCao", "MGMT_SakurabaEma", "MGMT_NikaidouHiro", "MGMT_SherryTachibana"]',
    ...
    socket.on("room-action", ({ sessionId, uuid, action, roomId, data }) => {
      ...
      if (result.success) {
        ...
        if (action === "addPlayer") {
          ...
          Object.keys(userRoomMap).forEach(uuid => {
            if (userRoomMap[uuid] === roomId) {
                delete userRoomMap[uuid];
            }
        });
        } 
            ...
            const targetSocketEntry = Object.entries(globalState.onlineUsers).find(([sid, u]) => u.uuid === targetUuid);
            if (targetSocketEntry) {
                ...
                if (targetSocket) {
                    ...
                    targetSocket.emit("op-feedback", { type: 'error', message: '你已被列车长请下站台' });
                }
            }
        } 
